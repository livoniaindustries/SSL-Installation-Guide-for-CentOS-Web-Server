  SSL Installation Guide for CentOS Web Server body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, sans-serif; line-height: 1.6; color: #24292e; max-width: 1000px; margin: 0 auto; padding: 20px; } h1, h2, h3 { border-bottom: 1px solid #eaecef; padding-bottom: 0.3em; } h1 { font-size: 2em; } code { font-family: 'SF Mono', Monaco, Consolas, 'Liberation Mono', Courier, monospace; background-color: #f6f8fa; padding: 0.2em 0.4em; border-radius: 3px; font-size: 85%; } pre { background-color: #f6f8fa; border-radius: 3px; padding: 16px; overflow: auto; line-height: 1.45; border: 1px solid #ddd; } pre code { background-color: transparent; padding: 0; white-space: pre; } .note { background-color: #fff5b1; border-left: 4px solid #f9c513; padding: 12px; margin: 16px 0; border-radius: 3px; } .warning { background-color: #ffeef0; border-left: 4px solid #d73a49; padding: 12px; margin: 16px 0; border-radius: 3px; } .info-box { background-color: #e6f3ff; border-left: 4px solid #0366d6; padding: 12px; margin: 16px 0; border-radius: 3px; } table { border-collapse: collapse; width: 100%; margin: 16px 0; } th, td { border: 1px solid #dfe2e5; padding: 8px 13px; } th { background-color: #f6f8fa; } .step { background-color: #f9f9f9; border-left: 4px solid #28a745; padding: 16px; margin: 20px 0; border-radius: 0 4px 4px 0; } .command-block { background-color: #2d2d2d; color: #f8f8f2; padding: 16px; border-radius: 4px; font-family: 'SF Mono', Monaco, Consolas, monospace; margin: 16px 0; overflow-x: auto; } .config-file { background-color: #f8f8f8; border: 1px solid #ddd; padding: 16px; border-radius: 4px; font-family: 'SF Mono', Monaco, Consolas, monospace; margin: 16px 0; overflow-x: auto; } .screenshot-placeholder { background-color: #f1f8ff; border: 2px dashed #0366d6; border-radius: 8px; padding: 30px; text-align: center; margin: 20px 0; color: #0366d6; font-weight: 600; } .qa-box { background-color: #f6f8fa; border: 1px solid #e1e4e8; border-radius: 4px; padding: 20px; margin: 20px 0; } .question { font-weight: 600; color: #d73a49; } .answer { margin-top: 10px; padding-left: 20px; border-left: 3px solid #28a745; }
...
Apache won't start after SSL configuration

Run `sudo httpd -t` to check for syntax errors. Check file paths in ssl.conf.

Certificate works but browser shows warning

This is normal for self-signed certificates. Use Let's Encrypt for trusted certs.

HTTPS works but redirect loops

Check your redirect rules for conflicts. Ensure you're not redirecting both ways.

Mixed content warnings

Update all resource URLs (images, CSS, JS) to use HTTPS or relative paths.

SSL certificate expired

For self-signed: generate new one. For Let's Encrypt: run `sudo certbot renew`.
